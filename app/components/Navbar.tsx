        ...
        <div className="container flex items-center justify-between">
            <a href="" className="text-xl font-bold text-primary flex items-center">
                <span className="relative z-10">
                    <span className="text-glow text-foreground">Jatin742</span> Portfolio
                </span>
            </a>
                ...
                <div className="flex flex-col items-center space-y-8 text-xl">
                    {navItems.map((item, key) => (
                        ...
                        <a onClick={() => setIsMenuOpen(false)}
                            key={key} href={item.href} className="text-foreground/80 hover:text-primary transition-colors duration-300">{item.name}</a>
                        // </a>
                    ))}
                </div>
            </div>
